...
void setup(){
  ...
  //Input Limit Current
  if (!bq25896.setIINLIM(600)) {
    Serial.println("Input current limit set to 600 mA.");
  } else {
    Serial.println("Failed to set input current limit.");
  }
  // Termination Current Limit
  if (!bq25896.setITERM(192)) {
    Serial.println("Termination current set to 192 mA.");
  } else {
    Serial.println("Failed to set termination current.");
  }
  // Precharge Current Limit
  // Pre-charge current is used when battery is below a certain voltage.
   if (!bq25896.setIPRECHG(128)) {
    Serial.println("Pre-charge current set to 128 mA.");
  } else {
    Serial.println("Failed to set pre-charge current.");
  }
  // Charge Voltage Limit
  if (!bq25896.setVREG(4000)) {
    Serial.println("Charge Voltage set to 4100mV.");
  } else {
...
void loop() {
  // Optionally, read parameters periodically
...
/*********************************************NOTE************************************************
PMIC Parameters whose values can be user defined and tested by using the above code methodology:
INLIM
ICHG
VREG
SYS_MIN
